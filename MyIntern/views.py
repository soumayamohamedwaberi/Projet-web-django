...
def inscription(request):
    """
    Page d'inscription avec choix du type d'utilisateur
    """
...
def inscription_etudiant(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            user = form.save()
            login(request, user)
            messages.success(request, f"Bienvenue {user.username} ! Votre compte étudiant a été créé avec succès.")
...
def inscription_entreprise(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            user = form.save()
            login(request, user)
            messages.success(request, f"Bienvenue ! Votre compte entreprise a été créé avec succès.")
            return redirect('dashboard')
    else:
...
def dashboard(request):
    """
    Dashboard principal qui redirige selon le type d'utilisateur
    """
    if request.user.user_type == 'etudiant':
        return redirect('dashboard_etudiant')
    elif request.user.user_type == 'entreprise':
